...
def draw_graph_from_csv(file):
    ...
    global filename
    filename = file
    df = pandas.read_csv("data/" + filename,na_filter=False)[['index_chain','paired','pair_type_LW']]
    ...
    for i in range(len(g.vs)):
        ...
        if pairedNode != "":
            ...
            if pairedNode.find(",") != -1:                       # la liste "paired" contient plus d'un élément
                ...
                if int(pairedNode) in range (1,len(g.vs)):
...
for i in range(780,820):
    result = draw_graph_from_csv(allCsvFiles[i])
    find_subgraph(g,rin_23,"rin_23")
    find_subgraph(g,rin_129,"rin_129")


print(list)
print("Pourcentage d'erreur : ")
print(len(list)/40)
